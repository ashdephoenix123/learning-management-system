...
const page = () => {
  ...
  const { courseDetails: { courseMatrix } } = allDetails || { courseDetails: { courseMatrix: '' } };

  return (
    <section className='min-h-screen card2'>
      <h3 className='heading1'>Course Matrix & Syllabus</h3>
      <Link href={courseMatrix} target='_blank' className='relative w-16 h-16 hover:cursor-pointer block mb-6'>
